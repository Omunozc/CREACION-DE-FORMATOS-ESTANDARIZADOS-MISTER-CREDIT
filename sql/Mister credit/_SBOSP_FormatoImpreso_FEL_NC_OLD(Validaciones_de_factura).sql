...
BEGIN
    ...
    BEGIN
        ...
        WHERE T0.DocEntry = @DocKey@
          ...
          AND (T0.U_FE_Status = '02' OR T0.U_FE_Contingencia = 'Y') 
          AND (T0.U_FE_IDGFace IS NOT NULL OR T0.U_FE_Contingencia = 'Y')
        GROUP BY 
            T0.DocNum, 
            T2.CardCode, 
            T0.U_Direccion, 
...
-- =======================================================================================
    ...
    BEGIN
        ...
        LEFT JOIN @PagosR T9 ON T0.DocEntry = T9.InvDocEntry AND T9.InvType = T0.ObjType
        LEFT JOIN OSLP T13 ON T0.SlpCode = T13.SlpCode
        JOIN OITL (NOLOCK) T31 ON T0.ObjType = T31.DocType AND T0.DocEntry = T31.DocEntry AND T31.DocType = 14  -- <== ¡AQUÍ SE CAMBIÓ A 14 PARA NOTA DE CRÉDITO!
